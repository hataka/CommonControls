...
{
	public class azukiJumpDialog : Form
	{
		...
		private void InitializeComponent()
		{
			...
			base.Name = "azukiJumpDialog";
			base.StartPosition = FormStartPosition.CenterParent;
			this.Text = "行へ移動";
			base.Load += new EventHandler(this.azukiJumpDialog_Load);
			base.ResumeLayout(false);
			base.PerformLayout();
		}

		public azukiJumpDialog()
		{
			this.InitializeComponent();
		}

		public azukiJumpDialog(AzukiControl txtBox)
		{
			this.InitializeComponent();
			this._textBox = txtBox;
		}

		private void azukiJumpDialog_Load(object sender, EventArgs e)
		{
		...
		{
			string[] array = this._textBox.Document.Text.Split(new char[]
			{
				'\n'
			});
			...
			else
			{
				...
				int num4 = stringBuilder.ToString().Length - (num2 - i);
				this._textBox.Document.SetSelection(num4, num4);
				this._textBox.Focus();
				base.Close();
				base.Dispose();
			}
		...
		{
			int num;
			this.textBox.Document.GetSelection(out num, out num);
			int length = num;
			string text = this._textBox.Text.Substring(0, length);
