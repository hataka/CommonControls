// -*- mode: cs -*-  Time-stamp: <2010-10-29 08:39:17 kahata>
/*================================================================
 * title: 
 * file: NeHeLesson09.cs
 * path: F:\VCSharp\CsGL-cs_graphics_library\NeHeLessons\NeHeLesson09\NeHeLesson09.cs
 * url:  http://localhost/VCSharp/CsGL-cs_graphics_library/NeHeLessons/NeHeLesson09/NeHeLesson09.cs
 * created: Time-stamp: <2010-10-29 08:39:17 kahata>
 * revision: $Id$
 * Programmed By: kahata
 * To compile:
 * To run: 
 * link: link: http://csgl.sourceforge.net/
 * link: http://nehe.gamedev.net/default.asp
 * link: http://nehe.gamedev.net/data/lessons/lesson.asp?lesson=09
 * description: 
 *================================================================*/
	...
	public sealed class NeHeLesson09 : Model {
		...
		private void LoadTextures() {
			/**
				...
				C# の string は System.String のエイリアスです。文字列は「”」で囲みますが、
				ファイルへのパスや SQL など複数行の文字列を扱いたい場合に、「@(アットマーク)」を利用すると便利です。
				とりあえず、C# で普通に文字を書くには、次の様に書きますね。
					string s = "C:\\path\\to\\file";
				「\」はエスケープする必要があるので、「\\」と重ねて書くわけです。
				「@」を利用すると次の様に書くことができ、便利になります。
					string s = @"C:\path\to\file";
				これを、「@quoted(クォート) string」と呼びます。
				エスケープシーケンスが処理されなくなることで上記の記述が可能になりますので、
				逆に文字列中に「”」を含めたい場合には、「”"」と二重にする必要が出てきます。
